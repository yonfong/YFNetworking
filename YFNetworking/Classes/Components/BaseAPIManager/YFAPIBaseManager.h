...
/*
 ...
 3.有一种情况是manager调用api时使用的参数不一定是明文传递的，有可能是从某个变量或者跨越了好多层的对象中来获得参数，那么在调用api的最后一关会有一个参数验证，当参数不对时不访问api，同时自身的errorType将会变为YFAPIManagerErrorTypeParamsError。这个机制可以优化我们的app。
...
/*
 ...
 YFAPIBaseManager会先调用这个函数，然后才会调用到 id<YFAPIManagerValidator> 中的 manager:isCorrectWithParamsData:
